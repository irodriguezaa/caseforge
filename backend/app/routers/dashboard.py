...
def get_qc_summary(
    ...
    if deliverable_ids:
        deliverable_names = dict(
            db.execute(
                select(Deliverable.id, Deliverable.name).where(Deliverable.id.in_(deliverable_ids))
            ).all()
        )
        deliverable_version_counts = dict(
            db.execute(
                select(Release.deliverable_id, func.count(Release.id))
                .where(Release.deliverable_id.in_(deliverable_ids))
                .group_by(Release.deliverable_id)
            ).all()
        )
        deliverable_revalidacion_counts = dict(
            db.execute(
                select(Release.deliverable_id, func.count(Release.id))
                .where(
                    Release.deliverable_id.in_(deliverable_ids),
                    Release.release_type == ReleaseType.REVALIDACION,
                )
                .group_by(Release.deliverable_id)
            ).all()
        )
        ...
        ordered_rows = db.execute(
            ...
            .where(Release.deliverable_id.in_(deliverable_ids))
            .order_by(Release.deliverable_id.asc(), Release.created_at.asc(), Release.id.asc())
        ).all()
        for release_id, deliverable_id in ordered_rows:
    ...
    for rel in tracked_releases:
        ...
        execution_items.append(
            ActivityItem(
                ...
                release_type=rel.release_type.value if rel.release_type else None,
                deliverable_release_ordinal=release_ordinal.get(rel.id) if rel.deliverable_id else None,
                deliverable_total_versions=(
                    deliverable_version_counts.get(rel.deliverable_id) if rel.deliverable_id else None
                ),
