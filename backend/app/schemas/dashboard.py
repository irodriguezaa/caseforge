...
class ActivityItem(BaseModel):
    ...
    percent_avance: float  # (PASS + FAIL + N_A) / planned TCs
    percent_cobertura: float  # (non-UNEXECUTED) / planned TCs
    brecha: float = 0.0  # percent_cobertura - percent_avance; share that is BLOCKED
...
class QcDashboardSummary(BaseModel):
    ...
    ActivityItem.percent_cobertura is non-UNEXECUTED / planned TCs.
    ActivityItem.percent_avance is (PASS + FAIL + N_A) / planned TCs (BLOCKED excluded).
