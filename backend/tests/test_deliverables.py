...
def test_evolutivo_does_not_require_parent(client) -> None:
    release = _create_release(
        client, version="1.0.0", deliverable_name="WEB - X", release_type="EVOLUTIVO"
    )
    assert release["release_type"] == "EVOLUTIVO"
    assert release["parent_release_id"] is None


def test_evolutivo_rejects_parent_release_id(client) -> None:
